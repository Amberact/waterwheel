...
class storage{
	...
	updateResource(waterwheeluuid,tankuuid,requestuuid,status){
		let newwaterwheel = this.getWaterWheel(waterwheeluuid)
		if(!newwaterwheel)return false
		if(status==='ready'||status==='done'||status==='error')newwaterwheel.updateResource(tankuuid,resourceuuid,status)
		return newwaterwheel
	}
